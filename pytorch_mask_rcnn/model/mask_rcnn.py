...
class MaskRCNN(nn.Module):
    ...
    def __init__(
        ...
        rpn_reg_weights=(1.0, 1.0, 1.0, 1.0),
    ...
    def __init__(self, in_channels, layers, dim_reduced, num_classes):
        ...
        for layer_idx, layer_features in enumerate(layers, 1):
            d["mask_fcn{}".format(layer_idx)] = nn.Conv2d(
                next_feature, layer_features, 3, 1, 1
            )
...
class ResBackbone(nn.Module):
    def __init__(self, backbone_name, pretrained):
        ...
        self.body = nn.ModuleDict(
            d for i, d in enumerate(body.named_children()) if i < 8
        )
        in_channels = 2048
        self.out_channels = 256

        self.inner_block_module = nn.Conv2d(in_channels, self.out_channels, 1)
        self.layer_block_module = nn.Conv2d(
            self.out_channels, self.out_channels, 3, 1, 1
        )
