...
def fastrcnn_loss(class_logit, box_regression, label, regression_target):
    ...
    box_reg_loss = (
        F.smooth_l1_loss(
            box_regression[box_idx, label], regression_target, reduction="sum"
        )
        / N
    )
...
def maskrcnn_loss(mask_logit, proposal, matched_idx, label, gt_mask):
    ...
    mask_target = roi_align(gt_mask, roi, 1.0, M, M, -1)[:, 0]
...
class RoIHeads(nn.Module):
    ...
    def select_training_samples(self, proposal, target):
        ...
        regression_target = self.box_coder.encode(
            gt_box[matched_idx[pos_idx]], proposal[pos_idx]
        )
    ...
    def fastrcnn_inference(self, class_logit, box_regression, proposal, image_shape):
        ...
        results = dict(
            boxes=torch.cat(boxes), labels=torch.cat(labels), scores=torch.cat(scores)
        )
        return results
    ...
    def forward(self, feature, proposal, image_shape, target):
        if self.training:
            (
                ...
                label,
                regression_target,
            ) = self.select_training_samples(proposal, target)
        ...
        if self.training:
            classifier_loss, box_reg_loss = fastrcnn_loss(
                class_logit, box_regression, label, regression_target
            )
            losses = dict(
                roi_classifier_loss=classifier_loss, roi_box_loss=box_reg_loss
            )
        else:
            result = self.fastrcnn_inference(
                class_logit, box_regression, proposal, image_shape
            )
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                mask_loss = maskrcnn_loss(
                    mask_logit, mask_proposal, pos_matched_idx, mask_label, gt_mask
                )
