...
class RegionProposalNetwork(nn.Module):
    ...
    def compute_loss(self, objectness, pred_bbox_delta, gt_box, anchor):
        ...
        regression_target = self.box_coder.encode(
            gt_box[matched_idx[pos_idx]], anchor[pos_idx]
        )

        objectness_loss = F.binary_cross_entropy_with_logits(
            objectness[idx], label[idx]
        )
        box_loss = (
            F.l1_loss(pred_bbox_delta[pos_idx], regression_target, reduction="sum")
            / idx.numel()
        )
    ...
    def forward(self, feature, image_shape, target=None):
        ...
        proposal = self.create_proposal(
            anchor, objectness.detach(), pred_bbox_delta.detach(), image_shape
        )
        if self.training:
            objectness_loss, box_loss = self.compute_loss(
                objectness, pred_bbox_delta, gt_box, anchor
            )
            return proposal, dict(
                rpn_objectness_loss=objectness_loss, rpn_box_loss=box_loss
            )
